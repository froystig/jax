...
def _python_pmap(
  ...
  def f_pmapped(*args, **kwargs):
    ...
    out_tree, out = f_pmapped_(*args, **kwargs)
    return tree_unflatten(out_tree(), out)
...
def _cpp_pmap(
  ...
  def cache_miss(*args, **kwargs):
    ...
    out_tree, out = f_pmapped_(*args, **kwargs)
    out_pytree_def = out_tree()
    out = tree_unflatten(out_pytree_def, out)

    ### Decide whether we can support the C++ fast path
    execute: Optional[functools.partial] = None
    execute = pxla.parallel_callable.most_recent_entry()
    # Tracing is not supported, but as long as we have an executable, we should
    # be able to use the C++ path.
    use_fastpath = execute is not None
