...
def _shard_map_physicalize_rule(ctx: physical.PhysicalizeContext,
                                *args_flat,
                                jaxpr: core.Jaxpr,
                                mesh, in_names, out_names, check_rep, rewrite, auto):
  del ctx
  with core.extend_axis_env_nd(mesh.shape.items()):
    phys_jaxpr = physical.physicalize_jaxpr(core.ClosedJaxpr(jaxpr, []))
  ...
  out_flat = shard_map_p.bind(
      ...
      auto=auto)
  return out_flat
physical.physicalize_rules[shard_map_p] = _shard_map_physicalize_rule
