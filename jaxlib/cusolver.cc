...
/*static*/ absl::StatusOr<SolverHandlePool::Handle> SolverHandlePool::Borrow(
  ...
  if (pool->handles_[stream].empty()) {
    JAX_RETURN_IF_ERROR(cusolverDnCreate(&handle));
  } else {
    handle = pool->handles_[stream].back();
    pool->handles_[stream].pop_back();
  }
  if (stream) {
    JAX_RETURN_IF_ERROR(cusolverDnSetStream(handle, stream));
  }
...
absl::StatusOr<std::pair<int, py::bytes>> BuildPotrfDescriptor(
    const py::dtype& dtype, bool lower, int b, int n) {
  ...
  if (b == 1) {
    switch (type) {
      case Type::F32:
        JAX_RETURN_IF_ERROR(cusolverDnSpotrf_bufferSize(handle->get(), uplo, n,
                                                        /*A=*/nullptr,
                                                        /*lda=*/n, &lwork));
        workspace_size = lwork * sizeof(float);
        break;
      case Type::F64:
        JAX_RETURN_IF_ERROR(cusolverDnDpotrf_bufferSize(handle->get(), uplo, n,
                                                        /*A=*/nullptr,
                                                        /*lda=*/n, &lwork));
        workspace_size = lwork * sizeof(double);
        break;
      case Type::C64:
        JAX_RETURN_IF_ERROR(cusolverDnCpotrf_bufferSize(handle->get(), uplo, n,
                                                        /*A=*/nullptr,
                                                        /*lda=*/n, &lwork));
        workspace_size = lwork * sizeof(cuComplex);
        break;
      case Type::C128:
        JAX_RETURN_IF_ERROR(cusolverDnZpotrf_bufferSize(handle->get(), uplo, n,
                                                        /*A=*/nullptr,
                                                        /*lda=*/n, &lwork));
...
absl::Status Potrf_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  if (buffers[1] != buffers[0]) {
    JAX_RETURN_IF_ERROR(cudaMemcpyAsync(
  ...
  if (d.batch == 1) {
    switch (d.type) {
      case Type::F32: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnSpotrf(handle->get(), d.uplo, d.n, a, d.n,
      ...
      case Type::F64: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnDpotrf(handle->get(), d.uplo, d.n, a, d.n,
      ...
      case Type::C64: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnCpotrf(handle->get(), d.uplo, d.n, a, d.n,
      ...
      case Type::C128: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnZpotrf(
    ...
    JAX_RETURN_IF_ERROR(cudaStreamSynchronize(stream));
    switch (d.type) {
      case Type::F32: {
        JAX_RETURN_IF_ERROR(cusolverDnSpotrfBatched(
      ...
      case Type::F64: {
        JAX_RETURN_IF_ERROR(cusolverDnDpotrfBatched(
            handle->get(), d.uplo, d.n, static_cast<double**>(workspace), d.n,
            info, d.batch));
        break;
      }
      case Type::C64: {
        JAX_RETURN_IF_ERROR(cusolverDnCpotrfBatched(
            handle->get(), d.uplo, d.n, static_cast<cuComplex**>(workspace),
            d.n, info, d.batch));
        break;
      }
      case Type::C128: {
        JAX_RETURN_IF_ERROR(cusolverDnZpotrfBatched(
...
absl::StatusOr<std::pair<int, py::bytes>> BuildGetrfDescriptor(
    const py::dtype& dtype, int b, int m, int n) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
  int lwork;
  switch (type) {
    case Type::F32:
      JAX_RETURN_IF_ERROR(cusolverDnSgetrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::F64:
      JAX_RETURN_IF_ERROR(cusolverDnDgetrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::C64:
      JAX_RETURN_IF_ERROR(cusolverDnCgetrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::C128:
      JAX_RETURN_IF_ERROR(cusolverDnZgetrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
  }
  ...
  switch (d.type) {
    case Type::F32: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnSgetrf(handle->get(), d.m, d.n, a, d.m,
                                             static_cast<float*>(workspace),
                                             ipiv, info));
        a += d.m * d.n;
    ...
    case Type::F64: {
      double* a = static_cast<double*>(buffers[1]);
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnDgetrf(handle->get(), d.m, d.n, a, d.m,
                                             static_cast<double*>(workspace),
                                             ipiv, info));
        a += d.m * d.n;
        ipiv += std::min(d.m, d.n);
    ...
    case Type::C64: {
      cuComplex* a = static_cast<cuComplex*>(buffers[1]);
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnCgetrf(handle->get(), d.m, d.n, a, d.m,
                                             static_cast<cuComplex*>(workspace),
                                             ipiv, info));
        a += d.m * d.n;
        ipiv += std::min(d.m, d.n);
    ...
    case Type::C128: {
      cuDoubleComplex* a = static_cast<cuDoubleComplex*>(buffers[1]);
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnZgetrf(
...
absl::StatusOr<std::pair<int, py::bytes>> BuildGeqrfDescriptor(
    const py::dtype& dtype, int b, int m, int n) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
  ...
  switch (type) {
    case Type::F32:
      JAX_RETURN_IF_ERROR(cusolverDnSgeqrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::F64:
      JAX_RETURN_IF_ERROR(cusolverDnDgeqrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::C64:
      JAX_RETURN_IF_ERROR(cusolverDnCgeqrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
    case Type::C128:
      JAX_RETURN_IF_ERROR(cusolverDnZgeqrf_bufferSize(handle->get(), m, n,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m, &lwork));
      break;
  }
...
absl::Status Geqrf_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  switch (d.type) {
    ...
    case Type::C64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnCgeqrf(
    ...
    case Type::C128: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnZgeqrf(
            handle->get(), d.m, d.n, a, d.m, tau,
            static_cast<cuDoubleComplex*>(workspace), d.lwork, info));
        a += d.m * d.n;
        tau += std::min(d.m, d.n);
...
// Returns the workspace size and a descriptor for a geqrf operation.
absl::StatusOr<std::pair<int, py::bytes>> BuildOrgqrDescriptor(
    const py::dtype& dtype, int b, int m, int n, int k) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
  int lwork;
  switch (type) {
    case Type::F32:
      JAX_RETURN_IF_ERROR(cusolverDnSorgqr_bufferSize(handle->get(), m, n, k,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m,
                                                      /*tau=*/nullptr, &lwork));
      break;
    case Type::F64:
      JAX_RETURN_IF_ERROR(cusolverDnDorgqr_bufferSize(handle->get(), m, n, k,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m,
                                                      /*tau=*/nullptr, &lwork));
      break;
    case Type::C64:
      JAX_RETURN_IF_ERROR(cusolverDnCungqr_bufferSize(handle->get(), m, n, k,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m,
                                                      /*tau=*/nullptr, &lwork));
      break;
    case Type::C128:
      JAX_RETURN_IF_ERROR(cusolverDnZungqr_bufferSize(handle->get(), m, n, k,
                                                      /*A=*/nullptr,
                                                      /*lda=*/m,
                                                      /*tau=*/nullptr, &lwork));
      break;
...
absl::Status Orgqr_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  switch (d.type) {
    ...
    case Type::C64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnCungqr(
    ...
    case Type::C128: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnZungqr(
            handle->get(), d.m, d.n, d.k, a, d.m, tau,
            static_cast<cuDoubleComplex*>(workspace), d.lwork, info));
        a += d.m * d.n;
        tau += d.k;
...
absl::StatusOr<std::pair<int, py::bytes>> BuildSyevdDescriptor(
    const py::dtype& dtype, bool lower, int b, int n) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
  ...
  switch (type) {
    case Type::F32:
      JAX_RETURN_IF_ERROR(cusolverDnSsyevd_bufferSize(
          handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n, /*W=*/nullptr,
          &lwork));
      break;
    case Type::F64:
      JAX_RETURN_IF_ERROR(cusolverDnDsyevd_bufferSize(
          handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n, /*W=*/nullptr,
          &lwork));
      break;
    case Type::C64:
      JAX_RETURN_IF_ERROR(cusolverDnCheevd_bufferSize(
          handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n, /*W=*/nullptr,
          &lwork));
      break;
    case Type::C128:
      JAX_RETURN_IF_ERROR(cusolverDnZheevd_bufferSize(
          handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n, /*W=*/nullptr,
          &lwork));
      break;
  }
...
absl::Status Syevd_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  switch (d.type) {
    ...
    case Type::C128: {
      ...
      double* w = static_cast<double*>(buffers[2]);
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnZheevd(
...
absl::StatusOr<std::pair<int, py::bytes>> BuildSyevjDescriptor(
    const py::dtype& dtype, bool lower, int batch, int n) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
  int lwork;
  syevjInfo_t params;
  JAX_RETURN_IF_ERROR(cusolverDnCreateSyevjInfo(&params));
  ...
  if (batch == 1) {
    switch (type) {
      case Type::F32:
        JAX_RETURN_IF_ERROR(cusolverDnSsyevj_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params));
        break;
      case Type::F64:
        JAX_RETURN_IF_ERROR(cusolverDnDsyevj_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params));
        break;
      case Type::C64:
        JAX_RETURN_IF_ERROR(cusolverDnCheevj_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params));
        break;
      case Type::C128:
        JAX_RETURN_IF_ERROR(cusolverDnZheevj_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params));
        break;
    }
    ...
    switch (type) {
      case Type::F32:
        JAX_RETURN_IF_ERROR(cusolverDnSsyevjBatched_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params, batch));
        break;
      case Type::F64:
        JAX_RETURN_IF_ERROR(cusolverDnDsyevjBatched_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params, batch));
        break;
      case Type::C64:
        JAX_RETURN_IF_ERROR(cusolverDnCheevjBatched_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params, batch));
        break;
      case Type::C128:
        JAX_RETURN_IF_ERROR(cusolverDnZheevjBatched_bufferSize(
            handle->get(), jobz, uplo, n, /*A=*/nullptr, /*lda=*/n,
            /*W=*/nullptr, &lwork, params, batch));
        break;
    }
  ...
  return {
      {lwork, PackDescriptor(SyevjDescriptor{type, uplo, batch, n, lwork})}};
}
  ...
  if (d.batch == 1) {
    switch (d.type) {
      case Type::F32: {
        ...
        float* w = static_cast<float*>(buffers[2]);
        JAX_RETURN_IF_ERROR(
            cusolverDnSsyevj(handle->get(), jobz, d.uplo, d.n, a, d.n, w,
      ...
      case Type::F64: {
        double* a = static_cast<double*>(buffers[1]);
        double* w = static_cast<double*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnDsyevj(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
      ...
      case Type::C64: {
        cuComplex* a = static_cast<cuComplex*>(buffers[1]);
        float* w = static_cast<float*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnCheevj(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
      ...
      case Type::C128: {
        cuDoubleComplex* a = static_cast<cuDoubleComplex*>(buffers[1]);
        double* w = static_cast<double*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnZheevj(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
    ...
    switch (d.type) {
      case Type::F32: {
        float* a = static_cast<float*>(buffers[1]);
        float* w = static_cast<float*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnSsyevjBatched(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
      ...
      case Type::F64: {
        double* a = static_cast<double*>(buffers[1]);
        double* w = static_cast<double*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnDsyevjBatched(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
      ...
      case Type::C64: {
        cuComplex* a = static_cast<cuComplex*>(buffers[1]);
        float* w = static_cast<float*>(buffers[2]);
        JAX_RETURN_IF_ERROR(cusolverDnCheevjBatched(
            handle->get(), jobz, d.uplo, d.n, a, d.n, w,
...
absl::StatusOr<std::pair<int, py::bytes>> BuildGesvdDescriptor(
    const py::dtype& dtype, int b, int m, int n, bool compute_uv,
    bool full_matrices) {
  Type type = DtypeToType(dtype);
  auto handle = SolverHandlePool::Borrow();
  if (!handle.ok()) return handle.status();
...
absl::Status Gesvd_(cudaStream_t stream, void** buffers, const char* opaque,
  ...
  switch (d.type) {
    case Type::F32: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnSgesvd(
    ...
    case Type::F64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnDgesvd(
            handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m, s, u, d.m, vt,
    ...
    case Type::C64: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnCgesvd(handle->get(), d.jobu, d.jobvt,
                                             d.m, d.n, a, d.m, s, u, d.m, vt,
                                             d.n, static_cast<cuComplex*>(work),
    ...
    case Type::C128: {
      ...
      for (int i = 0; i < d.batch; ++i) {
        JAX_RETURN_IF_ERROR(cusolverDnZgesvd(
            handle->get(), d.jobu, d.jobvt, d.m, d.n, a, d.m, s, u, d.m, vt,
...
absl::StatusOr<std::pair<int, py::bytes>> BuildGesvdjDescriptor(
  ...
  if (batch == 1) {
    switch (type) {
      case Type::F32:
        ...
        break;
      case Type::F64:
        JAX_RETURN_IF_ERROR(cusolverDnDgesvdj_bufferSize(
            handle->get(), jobz, /*econ=*/0, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params));
        break;
      case Type::C64:
        JAX_RETURN_IF_ERROR(cusolverDnCgesvdj_bufferSize(
            handle->get(), jobz, /*econ=*/0, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params));
        break;
      case Type::C128:
        JAX_RETURN_IF_ERROR(cusolverDnZgesvdj_bufferSize(
            handle->get(), jobz, /*econ=*/0, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params));
    ...
    switch (type) {
      case Type::F32:
        JAX_RETURN_IF_ERROR(cusolverDnSgesvdjBatched_bufferSize(
            handle->get(), jobz, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
        ...
        break;
      case Type::F64:
        JAX_RETURN_IF_ERROR(cusolverDnDgesvdjBatched_bufferSize(
            handle->get(), jobz, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params, batch));
        break;
      case Type::C64:
        JAX_RETURN_IF_ERROR(cusolverDnCgesvdjBatched_bufferSize(
            handle->get(), jobz, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params, batch));
        break;
      case Type::C128:
        JAX_RETURN_IF_ERROR(cusolverDnZgesvdjBatched_bufferSize(
            handle->get(), jobz, m, n,
            /*A=*/nullptr, /*lda=*/m, /*S=*/nullptr,
            /*U=*/nullptr, /*ldu=*/m, /*V=*/nullptr,
            /*ldv=*/n, &lwork, params, batch));
  ...
  if (d.batch == 1) {
    switch (d.type) {
      case Type::F32: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnSgesvdj(
      ...
      case Type::F64: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnDgesvdj(
            handle->get(), d.jobz, /*econ=*/0, d.m, d.n, a, d.m, s, u, d.m, v,
      ...
      case Type::C64: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnCgesvdj(
            handle->get(), d.jobz, /*econ=*/0, d.m, d.n, a, d.m, s, u, d.m, v,
      ...
      case Type::C128: {
        ...
        JAX_RETURN_IF_ERROR(cusolverDnZgesvdj(
            handle->get(), d.jobz, /*econ=*/0, d.m, d.n, a, d.m, s, u, d.m, v,
    ...
    switch (d.type) {
      case Type::F32: {
        ...
        float* u = static_cast<float*>(buffers[3]);
        float* v = static_cast<float*>(buffers[4]);
        JAX_RETURN_IF_ERROR(cusolverDnSgesvdjBatched(
      ...
      case Type::F64: {
        ...
        double* u = static_cast<double*>(buffers[3]);
        double* v = static_cast<double*>(buffers[4]);
        JAX_RETURN_IF_ERROR(cusolverDnDgesvdjBatched(
            handle->get(), d.jobz, d.m, d.n, a, d.m, s, u, d.m, v, d.n,
            static_cast<double*>(work), d.lwork, info, params, d.batch));
        break;
      }
      case Type::C64: {
        ...
        cuComplex* u = static_cast<cuComplex*>(buffers[3]);
        cuComplex* v = static_cast<cuComplex*>(buffers[4]);
        JAX_RETURN_IF_ERROR(cusolverDnCgesvdjBatched(
            handle->get(), d.jobz, d.m, d.n, a, d.m, s, u, d.m, v, d.n,
            static_cast<cuComplex*>(work), d.lwork, info, params, d.batch));
        break;
      }
      case Type::C128: {
        ...
        cuDoubleComplex* u = static_cast<cuDoubleComplex*>(buffers[3]);
        cuDoubleComplex* v = static_cast<cuDoubleComplex*>(buffers[4]);
        JAX_RETURN_IF_ERROR(cusolverDnZgesvdjBatched(
            handle->get(), d.jobz, d.m, d.n, a, d.m, s, u, d.m, v, d.n,
            static_cast<cuDoubleComplex*>(work), d.lwork, info, params,
            d.batch));
        break;
